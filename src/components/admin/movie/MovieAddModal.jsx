...
import { Check, Crown, X, Upload, ImagePlus } from 'lucide-react';
...
import { GenrePickerField, BackdropGalleryField, backdropStateToDto } from './GenreAndImageFields';
...
/**
 * Ô ảnh: cho phép dán URL trực tiếp HOẶC upload file (tự động gọi
 * movieService.uploadImage rồi điền URL trả về vào ô).
 */
function ImageField({ label, imageType, value, onChange }) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      ...
      {value && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 2 }}>
          <img src={value} alt="" style={{ width: 34, height: imageType === 'backdrop' ? 20 : 48, objectFit: 'cover', borderRadius: 6, border: `1px solid ${T.border}` }} />
          <span style={{ fontFamily: FONT, fontSize: 11, color: T.textMuted }}>Xem trước</span>
        </div>
      )}
    </div>
  );
        ...
        <>
          ...
          >
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: 20, background: T.surface, display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <ImageField label="Poster" imageType="poster" value={form.posterUrl} onChange={set('posterUrl')} />
              <ImageField label="Backdrop (ảnh bìa)" imageType="backdrop" value={form.backdropUrl} onChange={set('backdropUrl')} />
