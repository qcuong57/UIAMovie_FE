// src/components/admin/movie/MovieDeleteModal.jsx
...
/**
 * Props:
 *   movie   – object { id, title, posterUrl, year, tmdbId } | null
 *   onClose – () => void
            ...
            <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              {movie?.posterUrl && (
                <div style={{ display: 'flex', gap: 14, alignItems: 'center', padding: '12px 14px', borderRadius: 10, background: T.surfaceAlt, border: `1px solid ${T.border}` }}>
                  ...
                  />
                  <div>
                    <p style={{ fontFamily: FONT, fontSize: 10.5, color: T.textMuted, marginBottom: 3, textTransform: 'uppercase', letterSpacing: '0.07em', fontWeight: 700 }}>Sắp xóa</p>
                    <p style={{ fontFamily: FONT, fontSize: 14, color: T.text, fontWeight: 700, lineHeight: 1.4 }}>{movie?.title}</p>
                    {movie?.tmdbId && <p style={{ fontFamily: FONT, fontSize: 11, color: T.textMuted, marginTop: 2 }}>TMDB #{movie.tmdbId}{movie?.year ? ` · ${movie.year}` : ''}</p>}
                  </div>
                </div>
              )}

              {/* Warning */}
              <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start', padding: '12px 14px', borderRadius: 10, background: '#FEF2F2', border: '1px solid rgba(220,38,38,0.2)' }}>
                <AlertTriangle size={16} color={T.red} style={{ flexShrink: 0, marginTop: 1 }} />
                <p style={{ fontFamily: FONT, fontSize: 13, color: '#991B1B', lineHeight: 1.65, margin: 0 }}>
                  Hành động này <strong>không thể hoàn tác</strong>. Toàn bộ dữ liệu liên quan đến phim này (video, bình luận, lịch sử xem) sẽ bị xóa vĩnh viễn.
                </p>
              </div>
