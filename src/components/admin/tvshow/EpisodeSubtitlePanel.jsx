// src/components/admin/tvshow/EpisodeSubtitlePanel.jsx
...
// ── Upload Tab ────────────────────────────────────────────────────────────────
function UploadTab({ episodeId, onRefresh }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await episodeSubtitleService.uploadSubtitle(episodeId, file, langCode, langName || undefined, isDefault);
...
// ── Translate Tab ─────────────────────────────────────────────────────────────
function TranslateTab({ episodeId, subtitles, onRefresh }) {
  ...
  const handleTranslate = async () => {
    ...
    try {
      const result = await episodeSubtitleService.translateSubtitle(episodeId, sourceId, targetCode, targetName || undefined);
...
// ── AI Generate Tab ───────────────────────────────────────────────────────────
function AiGenerateTab({ episodeId, onRefresh }) {
  ...
  const handleGenerate = async () => {
    ...
    setLoading(true); setError(''); setInfo('');
    try {
      const result = await episodeSubtitleService.aiGenerateSubtitle(episodeId, content.trim(), srcCode, tgtCode, tgtName || undefined);
...
// ── Subtitle Row ─────────────────────────────────────────────────────────────
function SubtitleRow({ subtitle, episodeId, onSetDefault, onDelete, onStatusUpdate }) {
  ...
  }, [isProcessing, subtitle.id, episodeId]);

  const handleDelete = async () => {
    if (!window.confirm(`Xóa subtitle "${subtitle.languageName || getLangName(subtitle.languageCode)}"?`)) return;
    setDeleting(true);
    try { await episodeSubtitleService.deleteSubtitle(episodeId, subtitle.id); onDelete(subtitle.id); }
  ...
  const handleSetDefault = async () => {
    ...
    setSettingDef(true);
    try { await episodeSubtitleService.setDefault(episodeId, subtitle.id); onSetDefault(subtitle.id); }
...
function AddSubtitleForm({ episodeId, subtitles, onRefresh }) {
            ...
            <AnimatePresence mode="wait">
              {mode === 'upload' && (
                <motion.div key="upload" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  <UploadTab episodeId={episodeId} onRefresh={() => { onRefresh(); setOpen(false); }} />
                </motion.div>
              )}
              {mode === 'translate' && (
                <motion.div key="translate" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  <TranslateTab episodeId={episodeId} subtitles={subtitles} onRefresh={() => { onRefresh(); setOpen(false); }} />
                </motion.div>
              )}
              {mode === 'aiGenerate' && (
                <motion.div key="ai" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  <AiGenerateTab episodeId={episodeId} onRefresh={() => { onRefresh(); setOpen(false); }} />
                </motion.div>
              )}
            </AnimatePresence>
          </div>
...
// ── Main export ───────────────────────────────────────────────────────────────
export default function EpisodeSubtitlePanel({ episodeId }) {
  ...
  const fetchSubtitles = useCallback(async () => {
    ...
    try {
      const data = await episodeSubtitleService.getSubtitles(episodeId);
  ...
  return (
    <div style={{ fontFamily: FONT }}>
      ...
      <AddSubtitleForm episodeId={episodeId} subtitles={subtitles} onRefresh={fetchSubtitles} />
