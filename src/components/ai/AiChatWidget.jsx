...
import { IconMovie, IconRefresh, IconX, IconMoodSmile, IconChevronDown, IconSend2 } from "@tabler/icons-react";
import aiService from "../../services/aiService";
import { FONT_BODY, FONT_DISPLAY } from "../../context/homeTokens";

import { W, MAX_CHAT_LENGTH, MAX_HISTORY, GREETING, INTENT_CHIPS, PROACTIVE_MESSAGES } from "./config/aiChatConfig";
import { ProactiveBubble, FabIcon, HeaderIconBtn, MoodPicker } from "./ui/AiChatUI";
...
export default function AiChatWidget() {
  ...
  useEffect(() => {
    if (open) {
      setTimeout(() => { scrollToBottom(false); inputRef.current?.focus(); }, 100);
      setHasUnread(false);
      setProactive(null);
    }
  }, [open, scrollToBottom]);
  ...
  useEffect(() => {
    ...
    if (!open && last?.role === "assistant" && messages.length > 1 && !last._typing) setHasUnread(true);
  }, [messages, open]);
  ...
  useEffect(() => {
    ...
    idleTimerRef.current = setTimeout(() => {
      if (!open) {
        ...
        const key  = Object.keys(PROACTIVE_MESSAGES).find(k => path.startsWith(k) && k !== "/") ?? (path === "/" ? "/" : "default");
  ...
  const buildHistory = (msgs) =>
    msgs.slice(1).filter(m => !m._typing).slice(-MAX_HISTORY).map(({ role, content }) => ({ role, content }));
  ...
  const sendMessage = async (text) => {
    ...
    try {
      const { reply, movies, intent, compareTable } = await aiService.chat(trimmed, buildHistory(nextMsgs));
      setLastIntent(intent || "movie");
      setMessages(prev => [
        ...prev.filter(m => !m._typing),
        { role: "assistant", content: reply, movies: movies?.length ? movies : undefined, intent: intent ?? "movie", compareTable: compareTable ?? null },
      ]);
    } catch {
      setMessages(prev => [
        ...prev.filter(m => !m._typing),
  ...
  const handleMoodSelect = async (mood) => {
    setShowMoodPicker(false);
    await sendMessage(`Tôi đang cảm thấy ${mood}, gợi ý phim phù hợp cho tôi nhé!`);
  };
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            style={{
              position: "fixed",
              bottom: 92, top: 72, right: 24,
              width: 372, zIndex: 9998,
              borderRadius: 20,
              background: W.bg,
          ...
          >
            ...
            <div style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{
                  fontFamily: FONT_DISPLAY, fontWeight: 800, fontSize: 13.5,
                  ...
                  <span style={{
                    ...
                    <motion.span
                      style={{ width: 5, height: 5, borderRadius: "50%", background: W.green, display: "block" }}
                      animate={{ opacity: [1, 0.35, 1] }}
                      transition={{ repeat: Infinity, duration: 2.2, ease: "easeInOut" }}
                    />
                    <span style={{ fontFamily: FONT_BODY, fontSize: 8.5, fontWeight: 600, color: W.green }}>Live</span>
                  </span>
                </div>
                <div style={{ fontFamily: FONT_BODY, fontSize: 10, color: W.textDim, marginTop: 1 }}>
                  Trợ lý tìm phim thông minh
                </div>
              </div>
              <div style={{ display: "flex", gap: 2, alignItems: "center" }}>
                <HeaderIconBtn onClick={() => setShowMoodPicker(v => !v)} title="Gợi ý theo tâm trạng" active={showMoodPicker} accentActive>
                  <IconMoodSmile size={15} />
                </HeaderIconBtn>
                {messages.length > 1 && (
                  <HeaderIconBtn onClick={clearChat} title="Cuộc trò chuyện mới">
                    <IconRefresh size={14} />
                  </HeaderIconBtn>
                )}
            ...
            <AnimatePresence>
              {showScrollDown && (
                <motion.button
                  ...
                  whileHover={{ y: -2, boxShadow: "0 10px 30px rgba(0,0,0,0.65)" }}
            ...
            <AnimatePresence>
              {isFirst && (
                <motion.div
                  ...
                  style={{ padding: "0 13px 8px", display: "flex", flexWrap: "wrap", gap: 5, flexShrink: 0 }}
                >
                  {currentChips.map((s, idx) => (
                    <motion.button
                      ...
                      transition={{ duration: 0.18, delay: idx * 0.05, ease: EASE_EXP }}
                    ...
                    >
                      <span style={{ fontSize: 11 }}>{s.emoji}</span> {s.label}
                    </motion.button>
                  ))}
                </motion.div>
              )}
            ...
            {/* Input area */}
            <div style={{ padding: "8px 12px 13px", flexShrink: 0, borderTop: `1px solid ${W.border}` }}>
              <div
                style={{
                  display: "flex", alignItems: "flex-end", gap: 8,
                  background: W.surfaceUp,
                  border: `1px solid ${overLimit ? W.warn : W.border}`,
                  borderRadius: 15, padding: "7px 7px 7px 14px",
              ...
              >
                <textarea
                  ...
                  style={{
                    ...
                    padding: "3px 0", color: W.text,
                    fontFamily: FONT_BODY, fontSize: 13, lineHeight: 1.55,
                    resize: "none", outline: "none", scrollbarWidth: "none",
                ...
                >
                  <IconSend2 size={14} color={canSend ? "#fff" : W.textDim} style={{ transform: "translateX(1px)" }} />
                </motion.button>
              </div>

              <div style={{ marginTop: 7, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <p style={{ fontFamily: FONT_BODY, fontSize: 9.5, color: W.textDim, letterSpacing: "0.02em", margin: 0 }}>
                ...
                {nearLimit && (
                  <span style={{
                    ...
                    flexShrink: 0, marginLeft: 8, transition: "color 0.15s",
                  }}>
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.88, transition: { type: "spring", stiffness: 380, damping: 18 } }}
        style={{
