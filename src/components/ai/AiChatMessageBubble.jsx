...
import { CompareCard, MovieCardsRow } from "./ui/AiChatMovies";

export default function AiChatMessageBubble({ role, content, isTyping, movies, onMovieClick, compareTable, intent }) {
    ...
    >
      ...
      {isUser ? (
        <div style={{
          ...
          <span style={{ fontFamily: FONT_BODY, fontSize: 9, fontWeight: 800, color: "#fff" }}>B</span>
        </div>
      ) : (
        <div style={{
          width: 24, height: 24, borderRadius: "50%",
      ...
      {/* Content */}
      <div style={{ maxWidth: isUser ? "72%" : "84%", display: "flex", flexDirection: "column", minWidth: 0 }}>
        {/* Bubble */}
        <div style={{
          padding: "9px 13px",
          ...
          fontFamily: FONT_BODY, fontSize: 13, lineHeight: 1.65,
          wordBreak: "break-word", whiteSpace: "pre-wrap",
          boxShadow: isUser
            ? "0 4px 16px rgba(229,24,30,0.2)"
            : "0 2px 8px rgba(0,0,0,0.25)",
        }}>
          {isTyping ? <TypingDots /> : (isUser ? content : renderMarkdown(content))}
        </div>

        {/* Compare card */}
        {!isUser && !isTyping && intent === "compare" && compareTable && movies?.length >= 2 && (
          <CompareCard movieA={movies[0]} movieB={movies[1]} markdownTable={compareTable} onMovieClick={onMovieClick} />
        )}
